    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <div className="relative mb-3 shrink-0">
          <Search
            size={14}
            className="absolute left-3.5 top-1/2 -translate-y-1/2 text-olive/40"
          />
          <input
            value={search}
            ...
            <table className="w-full">
              <thead className="sticky top-0 bg-white z-10">
                <tr className="border-b border-olive/10 text-left">
                  <th className="px-4 py-3 text-[10px] text-olive/40 uppercase tracking-widest font-medium">
                    Nombre
                  </th>
                  <th className="px-4 py-3 text-[10px] text-olive/40 uppercase tracking-widest font-medium">
                    Teléfono
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="text-center py-12 text-olive/30 text-sm italic"
                    >
                      {search
                        ? "Sin resultados"
        ...
        <div className="md:hidden flex-1 overflow-y-auto divide-y divide-olive/8 pb-24">
          {filtered.length === 0 ? (
            <p className="py-8 text-center text-sm text-olive/40">
              Sin resultados
            </p>
          ) : (
            filtered.map((c) => {
              const lastVisit = c.appointments[0]
                ? formatDate(c.appointments[0].date)
                : "—";
              return (
                <div
                  key={c.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-olive/50 mt-0.5">
                      {formatPhone(c.phone)}
                    </div>
                  </div>
                  {/* Right side */}
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-xs text-olive/40 mt-1">
                      {lastVisit}
                    </div>
                  </div>
