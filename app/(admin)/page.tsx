...
export default async function DashboardPage() {
  ...
  const apptChange = pctChange(
    metrics.appointmentsThisMonth,
    metrics.appointmentsLastMonth,
  );
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6 space-y-4">
      {/* Header */}
      <div>
        <h1 className="font-display text-3xl text-olive italic font-bold capitalize">
          {monthLabel}
        </h1>
          ...
          {metrics.todayAppointments.length === 0 ? (
            ...
            <div className="divide-y divide-olive/5 max-h-64 overflow-y-auto">
              {metrics.todayAppointments.map((a, i) => {
                ...
                const isNext =
                  a.status === "CONFIRMED" && i === firstConfirmedIdx;
                const borderColor =
                  a.status === "COMPLETED"
                    ? "border-l-moss"
                    : a.status === "CANCELLED"
                ...
                return (
                  ...
                  >
                    <div
                      className={`text-[11px] font-bold shrink-0 w-9 ${
                        a.status === "COMPLETED"
                          ? "text-moss"
                          : a.status === "CANCELLED"
                            ? "text-olive/30"
            ...
            <div className="divide-y divide-olive/5 max-h-64 overflow-y-auto">
              {metrics.lowStock.map((p) => (
                <div key={p.id} className="px-4 py-3">
                  <div className="text-xs font-medium text-olive truncate">
                    {p.name}
                  </div>
          ...
          <div className="divide-y divide-olive/5 max-h-64 overflow-y-auto">
            {metrics.revenueByService.map((item) => {
              ...
              return (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-olive truncate">
                      {item.service}
                    </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-card px-4 py-3 shrink-0 min-w-[160px] md:min-w-0">
      ...
      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
        ...
        {change && (
          <span
            className={`flex items-center gap-0.5 text-[11px] font-medium ${change.up ? "text-moss" : "text-blossom-dark"}`}
          >
