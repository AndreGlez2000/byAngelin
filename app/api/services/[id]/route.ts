...
export async function PATCH(
  ...
  if (Array.isArray(products)) {
    if (products.length === 0) {
      ...
      costIngredients = products.reduce(
        (sum: number, p: { productId: string; quantityUsed: number }) => {
          const prod = priceMap[p.productId];
          if (!prod) return sum;
          return (
            sum + (p.quantityUsed * prod.costPerUnit) / prod.capacityPerUnit
          );
        },
        0,
      );
    ...
    await db.$transaction([
      db.serviceProduct.deleteMany({ where: { serviceId: id } }),
      ...(products.length > 0
        ? [
            db.serviceProduct.createMany({
              data: products.map(
                (p: { productId: string; quantityUsed: number }) => ({
                  serviceId: id,
  ...
  const service = await db.service.update({
    ...
    data: {
      ...rest,
      ...(costIngredients !== undefined ? { costIngredients } : {}),
    },
    include: {
      serviceProducts: {
        include: {
